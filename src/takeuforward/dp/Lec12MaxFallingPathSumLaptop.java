...
public class Lec12MaxFallingPathSumLaptop {
    public static void main(String[] args) {
        ...
        System.out.println(getMaxPathSumLap(matrix1));
        System.out.println(getMaxPathSumLap(matrix2));
    }

    public static int getMaxPathSumLap(int[][] matrix) {
        int result = Integer.MIN_VALUE;
        for (int col = 0; col < matrix[0].length; col++) {
            int temp = recursionLap(0, col, matrix);
            result = Math.max(temp, result);
        }
        return result;
    }

    private static int recursionLap(int row, int col, int[][] matrix) {
        if (col < 0 || col == matrix[0].length) return - (int) Math.pow(10,8);
        if (row == matrix.length-1) return matrix[row][col];

        int down = matrix[row][col] + recursionLap(row+1,col,matrix);

        int leftDiagonal = matrix[row][col] + recursionLap(row+1,col-1,matrix);

        int rightDiagonal = matrix[row][col] + recursionLap(row+1,col+1,matrix);
